...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'football-stats-secret-key-2024-real-data'
...
# Use real data files
DATA_FILE = 'player_statistics_real.csv'
MODEL_FILE = 'player_performance_model_real.pkl'


def load_app_data():
    """Load model and real data on startup"""
    global model, df
    
    # Load real data
    if os.path.exists(DATA_FILE):
        df = pd.read_csv(DATA_FILE)
        print(f"✅ Loaded REAL data: {len(df)} records, {df['player_name'].nunique()} players")
    else:
        return False, f"Real data file not found: {DATA_FILE}. Please run train_model_real.py first."
    
    # Load model
    model = PlayerPerformanceModel()
    if os.path.exists(MODEL_FILE):
        model.load_model(MODEL_FILE)
        print(f"✅ Loaded model trained on REAL data")
    else:
        return False, f"Model not found: {MODEL_FILE}. Please run train_model_real.py first."
    
    return True, "Real data and model loaded successfully"
...
def get_players():
    """API endpoint to get all real players"""
    if df is None:
...
def get_player_stats(player_name):
    """API endpoint to get real player statistics"""
    if df is None:
        return jsonify({'error': 'Data not loaded'}), 500
    ...
    stats = {
        ...
        'league': player_data['league'].iloc[0] if 'league' in player_data.columns else 'Unknown',
...
if __name__ == '__main__':
    print("=" * 60)
    print("⚽ Football Statistics - REAL 2024-2025 Season Data")
    print("=" * 60)
    print("\nStarting server...")
